...
async def delete_cube_team(
        request: Request,
        team_code: Optional[str] = None, user=Depends(deps.get_current_user),
    ...
    if envs:
        for env in envs:
            await env_delete_service.logic_delete_by_env_id(request=request, session=session, user=user, env=env,
