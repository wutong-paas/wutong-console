    ...
    if pod_name != "all":
        ...
        if pods_info:
            for value in pods_info.values():
                total_memory_usage += round(value["memory_usage"], 2)
                total_usage_rate += round(value["memory_rate"], 2)
                total_cpu_usage += round(value["cpu_usage"], 2)
                total_cpu_rate += round(value["cpu_rate"], 2)
