...
class AlarmRobotRepo(BaseRepository[AlarmRobot]):
    ...
    def get_alarm_robot_by_code(self, session, alarm_robot_code):
        return session.execute(select(AlarmRobot).where(AlarmRobot.robot_code == alarm_robot_code)).scalars().first()

    def get_all_alarm_robot(self, session, team_code):
        if team_code:
            return session.execute(select(AlarmRobot).order_by(
            AlarmRobot.create_time.desc()).where(AlarmRobot.team_code == team_code)).scalars().all()
        return session.execute(select(AlarmRobot).order_by(
            AlarmRobot.create_time.desc())).scalars().all()
