...
class ServiceEnvVarRepository(BaseRepository[ComponentEnvVar]):
    ...
    def get_service_env_by_tenant_id_and_service_id(self, session, tenant_id, service_id):
        return (session.execute(
            select(ComponentEnvVar).where(ComponentEnvVar.tenant_id == tenant_id,
                                          ComponentEnvVar.service_id == service_id))).scalars().all()

    def get_service_env(self, session, tenant_id, service_id, scopes, is_change,
    ...
    def get_service_env_by_port(self, session, tenant_id, service_id, port):
        return (session.execute(
            select(ComponentEnvVar).where(ComponentEnvVar.tenant_id == tenant_id,
                                          ComponentEnvVar.service_id == service_id,
                                          ComponentEnvVar.container_port == port))).scalars().all()

    def get_env_by_ids_and_attr_names(self, session, tenant_id, service_ids, attr_names):
        return (session.execute(
            select(ComponentEnvVar).where(ComponentEnvVar.tenant_id == tenant_id,
    ...
    def get_service_env_by_scope(self, session, tenant_id, service_id):
        return (session.execute(
            select(ComponentEnvVar).where(ComponentEnvVar.tenant_id == tenant_id,
                                          ComponentEnvVar.service_id == service_id,
                                          ComponentEnvVar.scope == "outer"))).scalars().all()

    def delete_service_env(self, session, tenant_id, service_id):
        session.execute(
            delete(ComponentEnvVar).where(ComponentEnvVar.tenant_id == tenant_id,
                                          ComponentEnvVar.service_id == service_id))

    def get_service_envs(self, session, tenant_id, service_id):
        return session.execute(
            select(ComponentEnvVar).where(ComponentEnvVar.tenant_id == tenant_id,
                                          ComponentEnvVar.service_id == service_id)).scalars().all()
