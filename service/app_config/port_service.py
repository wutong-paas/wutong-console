...
from core.utils.constants import ServicePortConstants
...
class AppPortService:
    ...
    def __check_params(self, session: SessionClass, action, container_port, protocol, port_alias, service_id):
        ...
        if action == "change_protocol":
            if not protocol:
                return 400, "端口协议不能为空"
        if port_alias:
            code, msg = self.check_port_alias(session=session, port_alias=port_alias)
            if code != 200:
                return code, msg
        return 200, "检测成功"

    def __list_stream_outer_urls(self, session: SessionClass, component):
        region = team_region_repo.get_region_by_region_name(session, component.service_region)
        if not region:
            return None
        port_domain = {}
    ...
    def __list_component_access_urls(self, session: SessionClass, component):
        port_domains = {}
        domains = domain_repo.get_service_domains(session, component.service_id)
        for d in domains:
    ...
    def list_access_infos(self, session: SessionClass, tenant, services):
        ...
        for svc in services:
            ...
            if svc_ports[svc.service_id]["http_outer_port"]:
                access_urls = self.__list_component_access_urls(session=session, component=svc)
                port_and_urls = self.__list_stream_outer_urls(session=session, component=svc)
    ...
    def get_access_info(self, session: SessionClass, tenant, service):
        data = []
        service_ports = port_repo.get_service_ports(session, tenant.tenant_id, service.service_id)
        # ①是否有端口
        ...
        access_type, data = self.__handle_port_info(session=session, tenant=tenant, service=service,
                                                    unopened_port=unopened_port, http_outer_port=http_outer_port,
                                                    http_inner_port=http_inner_port,
                                                    stream_outer_port=stream_outer_port,
                                                    stream_inner_port=stream_inner_port)
        return access_type, data

    def __handle_port_info(self, session: SessionClass, tenant, service, unopened_port, http_outer_port,
                           http_inner_port,
                           stream_outer_port,
                           stream_inner_port):
        # 有http对外访问端口
        if http_outer_port:
            ...
            for p in http_outer_port:
                port_dict = p.__dict__
                access_urls = self.__get_port_access_url(session=session, tenant=tenant, service=service,
                                                         port=p.container_port)
                if not access_urls:
                    port_and_url = self.__get_stream_outer_url(session=session, tenant=tenant, service=service, port=p)
                    if port_and_url:
        ...
        if stream_outer_port:
            ...
            for p in stream_outer_port:
                port_and_url = self.__get_stream_outer_url(session=session, tenant=tenant, service=service, port=p)
                associate_info = self.get_port_associated_env(session=session, tenant=tenant, service=service,
                                                              port=p.container_port)
                port_dict = p.__dict__
        ...
        if stream_inner_port:
            ...
            for p in stream_inner_port:
                port_dict = p.__dict__
                env_list = self.get_port_associated_env(session=session, tenant=tenant, service=service,
                                                        port=p.container_port)
    ...
    def get_port_associated_env(self, session: SessionClass, tenant, service, port):
        ...
        # 对外且不可改env
        self_define_env_list = env_var_service.get_self_define_env(session=session, service=service)
        # todo list filter
        # both_outer_fix_env = self_define_env_list.filter(scope__in=("outer", "both"),
        #                                                  is_change=False).exclude(attr_name__in=attr_names)
        both_outer_fix_env = self_define_env_list
    ...
    def check_port(self, session: SessionClass, service, container_port):
        ...
        if not (1 <= container_port <= 65535):
            raise AbortRequest("component port out of range", msg_show="端口必须为1到65535的整数", status_code=412,
                               error_code=412)

    def check_port_alias(self, session: SessionClass, port_alias):
        if not port_alias:
            return 400, "端口别名不能为空"
    ...
    def add_service_port(self, session: SessionClass,
        ...
        self.check_port(session=session, service=service, container_port=container_port)
        ...
        new_port = port_repo.add_service_port(session, **service_port)
        # 第三方组件在添加端口是添加一条默认的健康检测数据
        if service.service_source == "third_party":
            tenant_service_ports = self.get_service_ports(session=session, service=service)
            port_list = []
    ...
    def get_port_variables(self, session: SessionClass, tenant, service, port_info):
        data = {"environment": []}
        ...
        if port_info.is_outer_service:
            ...
            if port_info.protocol != 'http' and port_info.protocol != "https":
                ...
                tcpdomain = region_repo.get_region_tcpdomain(session=session, region_name=cur_region)
                ...
                httpdomain = region_repo.get_region_httpdomain(session, service.service_region)
                domain = httpdomain
                port = 80
    ...
    def manage_port(self, session: SessionClass,
        ...
        code, msg = self.__check_params(session=session, action=action, container_port=container_port,
                                        protocol=protocol, port_alias=port_alias, service_id=service.service_id)
        if code != 200:
            return code, msg, None
    ...
    def update_by_k8s_services(self, session, tenant, region_name, app: Application, k8s_services):
        """
        ...
        ports = port_repo.list_by_service_ids(session, tenant.tenant_id, component_ids)
        ...
        k8s_services = {k8s_service["service_id"] + str(k8s_service["port"]): k8s_service for k8s_service in
                        k8s_services}
        for port in ports:
