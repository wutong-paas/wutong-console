...
from models.application.plugin import TeamPlugin, TeamComponentPluginRelation
from .component import Component
from .new_app import NewApp
from .original_app import OriginalApp
from ..app_config.promql_service import promql_service
from ..app_config.service_monitor_service import service_monitor_service
...
class PropertyChanges(object):
    def __init__(self, session, components: [Component], plugins: [TeamPlugin], app_template, support_labels):
        self.components = components
        self.plugins = plugins
        self.app_template = app_template
        self.support_labels = support_labels
        self.changes = self._get_component_changes(session, components, app_template)
    ...
    def _get_component_changes(self, session, components, app_template):
        cpt_changes = []
        for cpt in components:
            ...
            if not tmpl:
                continue
            cpt_changes.append(self._get_component_change(session, cpt, tmpl))

        return cpt_changes

    def _get_component_change(self, session, component: Component, component_tmpl: map):
    ...
    def _plugin_deps(self, old_plugin_deps: [TeamComponentPluginRelation], plugin_deps):
        if not plugin_deps:
            return None
        add = []
