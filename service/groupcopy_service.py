...
class GroupAppCopyService(object):
    ...
    def get_group_services_with_build_source(self, session, tenant, region_name, group_id):
        ...
        for group_service in group_services:
            group_service = dict(group_service)
            group_service["app_name"] = group_service["group_name"]
        ...
        for service_id in service_ids:
            ...
            change_services.update(
                {service_id: {
                    ...
                    "ServiceAlias": application_service.create_service_alias(session, new_service_id)
                }})
                ...
                if changes[service["service_base"]["service_id"]].get("build_source"):
                    ...
                    if version:
                        if service["service_base"]["service_source"] == "source_code":
                            ...
                            image_list = service["service_base"]["image"].split(":")[0: -1]
                            image = ":".join(image_list)
                            service["service_base"]["image"] = image + ":" + version
    ...
    def copy_group_services(self, session, user, old_team, old_region_name, tar_team, tar_region_name, tar_group,
                            group_id,
                            choose_services):
        changes = {}
        service_ids = []
        ...
        services_metadata, change_services_map = self.get_modify_group_metadata(session, old_team, old_region_name,
                                                                                ...
                                                                                changes)
        self.save_new_group_app(session, user, tar_team, tar_region_name, tar_group.ID,
                                services_metadata, change_services_map, old_team == tar_team,
                                old_region_name == tar_region_name)
        return groupapp_copy_service.build_services(session, user, tar_team, tar_region_name, tar_group.ID,
                                                    change_services_map)

    def save_new_group_app(self, session, user, tar_team, region_name, group_id, metadata, changed_service_map,
                           same_team, same_region):
        migrate_service.save_data(session, tar_team, region_name, user, changed_service_map, metadata, group_id,
                                  same_team, same_region)
        ...
        for service in services:
            if service.service_id in change_service_ids:
                ...
                if build_error_plugin_ids:
                    app_plugin_relation_repo.delete_service_plugin_relation_by_service_id(
                        session, service.service_id, build_error_plugin_ids)
        return result
