...
class YamlService(object):
    ...
    def get_all_yaml_info(self, yaml_data_list):
        ...
        persistent_volume_claim_data = data.get("PersistentVolumeClaim", None)
        horizontal_pod_auto_scaler_data = data.get("HorizontalPodAutoscaler", None)
        ...
        for commpoent_data in commpoent_datas:
            ...
            label_name = match_labels.get("app", None)
            ...
            for container in containers:
                ...
                for port in ports:
                    ...
                    if service_data:
                        for service in service_data:
                            ...
                            target_pod_name = service["spec"]["selector"]["app"]
                            ...
                            for port_service in ports_service:
                                ...
                                if target_pod_name == label_name:
                                    if deployment_port == port_service_target_port or port_name == port_service_target_port:
                                        if node_port:
                                            outer_enable = True
                ...
                if persistent_volume_claim_data:
                    for persistent_volume_claim in persistent_volume_claim_data:
                        persistent_volume_claim_name = persistent_volume_claim.get("metadata").get("name")
                        for volume in volumes:
                            volume_persistent = volume.get("persistentVolumeClaim")
                            if volume_persistent:
                                volume_persistent_name = volume_persistent.get("claimName")
                                if persistent_volume_claim_name == volume_persistent_name:
                                    volume_claim_info = persistent_volume_claim.get('spec')

                if horizontal_pod_auto_scaler_data:
