...
class AppRestore(object):
    def __init__(self, tenant_env, service):
    ...
    def svc_source(self, session, service_source):
        if not service_source:
            ...
            return
        service_source_repo.delete_service_source(session=session, env_id=self.tenant_env.env_id,
    ...
    def envs(self, session, service_env_vars):
        env_var_repo.delete_service_env(session, self.tenant_env.env_id, self.service.service_id)
        if service_env_vars:
            envs = []
    ...
    def ports(self, session, service_ports):
        port_repo.delete_service_port(session, self.tenant_env.env_id, self.service.service_id)
        if service_ports:
            ports = []
    ...
    def dep_services(self, session, service_relation):
        dep_relation_repo.delete_service_relation(session, self.tenant_env.env_id, self.service.service_id)
