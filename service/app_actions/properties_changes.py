...
from service.rbd_center_app_service import rbd_center_app_service
...
class PropertiesChanges(object):
    ...
    def get_property_changes(self, session, template, level="svc"):
        ...
        envs = self.env_changes(session, component.get("service_env_map_list", []))
        if envs:
            result["envs"] = envs
        ports = self.port_changes(session, component.get("port_map_list", []))
        if ports:
            result["ports"] = ports
        connect_infos = self.env_changes(session, component.get("service_connect_info_map_list", []))
        if connect_infos:
            result["connect_infos"] = connect_infos
        volumes = self.volume_changes(session, component.get("service_volume_map_list", []))
        if volumes:
        ...
        dep_services = self.dep_services_changes(session, component, dep_uuids, component_names, level)
        if dep_services:
            result["dep_services"] = dep_services
        dep_volumes = self.dep_volumes_changes(session, component.get("mnt_relation_list", []))
        if dep_volumes:
        ...
        component_graphs = self.component_graph_changes(session, component.get("component_graphs", []))
        if component_graphs:
            result["component_graphs"] = component_graphs

        component_monitors = self.component_monitor_changes(session, component.get("component_monitors", []))
    ...
    def component_graph_changes(self, session, component_graphs):
        if not component_graphs:
            return None

        old_graphs = component_graph_repo.list(session, self.service.service_id)
    ...
    def component_monitor_changes(self, session, service_monitors):
    ...
    def env_changes(self, session, envs):
        """
    ...
    def dep_services_changes(self, session, component, dep_uuids, component_names={}, level="svc"):
        """
        ...
        service_shares = service_source_repo.get_service_sources_by_service_ids(session=session,
                                                                                service_ids=service_ids)
        service_share_uuids = [service_share.service_share_uuid for service_share in service_shares]
    ...
    def new_dep_services_from_apps(self, component, dep_uuids, component_names={}):
        ...
        for dep_service in component.get("dep_service_map_list", []):
            ...
            if service_share_uuid not in dep_uuids:
                continue
            result.append(
                {"service_share_uuid": service_share_uuid, "service_cname": component_names.get(service_share_uuid)})
        return result

    def port_changes(self, session, new_ports):
    ...
    def volume_changes(self, session, new_volumes):
        if not new_volumes:
            return
        old_volumes = volume_repo.get_service_volumes_with_config_file(session, self.service.service_id)
        ...
        for new_volume in new_volumes:
            ...
            if not new_volume.get("file_content"):
                continue
            old_file_content = volume_repo.get_service_config_file(session, old_volume)
            if old_file_content and old_file_content.file_content != new_volume["file_content"] or old_volume.mode != new_volume.get(
                    "mode"):
    ...
    def plugin_changes(self, session, new_plugins):
        if not new_plugins:
            return None
    ...
    def dep_volumes_changes(self, session, new_dep_volumes):
