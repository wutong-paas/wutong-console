...
from service.app_actions.app_restore import AppRestore
from service.app_actions.exception import ErrBackupNotFound
from service.app_actions.properties_changes import PropertiesChanges, get_upgrade_app_template, get_template_component
...
class MarketService(object):
    ...
    def dummy_func(self, session, changes):
    ...
    def set_properties(self, typ3=PropertyType.ALL.value):
        ...
        m = {
            PropertyType.ORDINARY.value: [
                "deploy_version", "app_version", "image", "slug_path", "envs", "connect_infos", "ports", "volumes",
                "probe",
    ...
    def pre_action(self, session):
        ...
        backup = self.create_backup(session)
        logger.info(
            "service id: {}; backup id: {}; backup successfully.".format(self.service.service_id, backup.backup_id))
        self.set_changes(session)
        ...
        try:
            ...
            self.restore_backup(session, backup)
    ...
    def set_changes(self, session):
        ...
        if not self.changes and self.changes != {} and pc:
            _, changes = pc.get_property_changes(session, template=template)
            logger.debug(
                "service id: {}; dest version: {}; changes: {}".format(self.service.service_id, self.version, changes))
            self.changes = changes

    def create_backup(self, session):
        """create_backup
        ...
        return service_backup_repo.base_create(session=session, add_model=TeamServiceBackup(**backup))

    def modify_property(self, session):
        """
        ...
        if component:
            if not self.update_source:
                self._update_service(session, component)
                self._update_service_source(session, component, self.version, self.template_update_time)
    ...
    def _update_service(self, session, app):
        ...
        session.merge(self.service)

    def _update_service_source(self, session, app, version, template_updatetime):
        new_extend_info = {}
        share_image = app.get("share_image", None)
    ...
    def _sync_outer_envs(self, session, envs):
        self._sync_envs(session, envs, "outer")

    def _sync_envs(self, session, envs, scope):
        """
        raise RegionApiBaseHttpClient.CallApiError
        """
        if envs is None:
        ...
        for env in add:
            ...
            try:
                ...
                raise remote_component_client.CallApiError(e.apitype, e.url, e.method, res, e.body)

    def _restore_inner_envs(self, session, backup):
        self._restore_envs(session, backup, "inner")

    def _restore_outer_envs(self, session, backup):
        self._restore_envs(session, backup, "outer")

    def _restore_envs(self, session, backup, scope):
        ...
        if not self.auto_restore:
            self.app_restore.envs(session, service_env_vars)
    ...
    def _sync_ports(self, session, ports):
        ...
        if envs:
            self._sync_outer_envs(session, envs)

    def _restore_ports(self, session, backup):
        backup_data = json.loads(backup.backup_data)
        if isinstance(backup_data, list):
        ...
        if not self.auto_restore:
            self.app_restore.ports(session, service_ports)
    ...
    def _update_volumes(self, session, volumes):
        ...
        for volume in volumes.get("upd"):
            ...
            if not file_content and volume["volume_type"] != "config-file":
                continue
            v = volume_repo.get_service_volume_by_name(session, self.service.service_id, volume["volume_name"])
            if not v:
    ...
    def _restore_volumes(self, session, backup):
        backup_data = json.loads(backup.backup_data)
        if isinstance(backup_data, list):
        ...
        if not self.auto_restore:
            self.app_restore.volumes(session, service_volumes, service_config_file)
    ...
    def _sync_probe(self, session, probe):
        """
        raise RegionApiBaseHttpClient.CallApiError
        """
    ...
    def _restore_probe(self, session, backup):
        backup_data = json.loads(backup.backup_data)
        if isinstance(backup_data, list):
        ...
        if not self.auto_restore:
            self.app_restore.probe(session, probe)
    ...
    def _restore_dep_services(self, session, backup):
        backup_data = json.loads(backup.backup_data)
        if isinstance(backup_data, list):
        ...
        if not self.auto_restore:
            self.app_restore.dep_services(session, service_relation)
    ...
    def _sync_dep_volumes(self, session, dep_volumes):
        def sync_dep_vol(dep_vol_info):
            """
            raise RegionApiBaseHttpClient.CallApiError
            """
            dep_vol = volume_repo.get_service_volume_by_name(session, dep_vol_info["service_id"],
                                                             dep_vol_info["mnt_name"])
            if dep_vol is None:
    ...
    def _restore_dep_volumes(self, session, backup):
        backup_data = json.loads(backup.backup_data)
        if isinstance(backup_data, list):
        ...
        if not self.auto_restore:
            self.app_restore.dep_volumes(session, dep_vols)
    ...
    def _update_plugins(self, session, plugins):
        ...
        for plugin in delete:
            app_plugin_service.delete_service_plugin_relation(session, self.service, plugin["plugin_id"])
            app_plugin_service.delete_service_plugin_config(session, self.service, plugin["plugin_id"])

    def _sync_plugins(self, session, plugins):
        """
        raise RegionApiBaseHttpClient.CallApiError
        """
    ...
    def _restore_plugins(self, session, backup):
        backup_data = json.loads(backup.backup_data)
        if isinstance(backup_data, list):
        ...
        if not self.auto_restore:
            self.app_restore.plugins(session, relations)
    ...
    def _restore_service(self, session, backup):
        backup_data = json.loads(backup.backup_data)
        if isinstance(backup_data, list):
        ...
        if not self.app_restore:
            self.app_restore.svc(session=session, service_base=service_base)

    def _restore_service_source(self, session, backup):
        backup_data = json.loads(backup.backup_data)
        if isinstance(backup_data, list):
        ...
        if not self.auto_restore:
            self.app_restore.svc_source(session=session, service_source=service_source)
