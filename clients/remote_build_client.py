...
from common.base_client_service import get_enterprise_region_info, get_region_access_info
...
class RemoteBuildClient(ApiBaseHttpClient):
    ...
    def get_env_services_deploy_version(self, session, region, tenant_env, data):
        ...
        url = url + "/v2/tenants/" + tenant_env.tenant_name + "/envs/" + tenant_env.env_name + "/deployversions"
    ...
    def get_pod(self, session, region_name, tenant_env, pod_name):
        ...
        url = url + "/v2/tenants/" + tenant_env.tenant_name + "/envs/" + tenant_env.env_name + "/pods/" + pod_name
    ...
    def check_resource_name(self, session, tenant_env, region_name, rtype, name):
        ...
        url = url + "/v2/tenants/" + tenant_env.tenant_name + "/envs/" + tenant_env.env_name + "/checkResourceName"
    ...
    def update_ingresses_by_certificate(self, session, region_name, tenant_env, body):
        ...
        """
        url, token = get_region_access_info(region_name, session)
        url = url + "/v2/tenants/" + tenant_env.tenant_name + "/envs/" + tenant_env.env_name + "/gateway/certificate"
        self._set_headers(token)
    ...
    def share_service(self, session, region, tenant_env, service_alias, body):
        ...
        url = "{0}/v2/tenants/{1}/envs/{2}/services/{3}/share".format(url, tenant_env.tenant_name,
                                                                      tenant_env.env_name,
                                                                      service_alias)
        self._set_headers(token)
        res, body = self._post(session, url, self.default_headers, region=region, body=json.dumps(body))
    ...
    def share_service_result(self, session, region, tenant_env, service_alias, region_share_id):
        ...
        url = "{0}/v2/tenants/{1}/envs/{2}/services/{3}/share/{4}".format(url, tenant_env.tenant_name,
                                                                          tenant_env.env_name,
                                                                          service_alias,
                                                                          region_share_id)
        self._set_headers(token)
